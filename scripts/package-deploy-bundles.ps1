...
if ([string]::IsNullOrWhiteSpace($cloudEnvId) -and (Test-Path -LiteralPath $envFile)) {
  $envLine = Get-Content -LiteralPath $envFile | Where-Object { $_ -match '^\s*VITE_CLOUDBASE_ENV_ID\s*=' } | Select-Object -First 1
  if ($envLine -match '=\s*(.+?)\s*$') { $cloudEnvId = $Matches[1].Trim() }
}
if ([string]::IsNullOrWhiteSpace($cloudEnvId)) {
  throw 'CLOUDBASE_ENV_ID or admin/.env.local VITE_CLOUDBASE_ENV_ID is required for a real admin static build.'
}
$compiledRoot = Join-Path $cloudRoot '.build'
$compiledEntry = Join-Path $compiledRoot 'wc-shop-function\index.js'
$compiledShared = Join-Path $compiledRoot 'shared'
